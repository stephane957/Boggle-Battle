...
""" Ces 2 fonctions sont en commentaire car elles ne sont pas encore achevées (faute de temps) mais
nécessaire au fonctionnement optimum du programme
Ce sont les fonctions qui sont censés empêcher de cliquer 2 fois de suite sur la même lettre
avec ma version et celle de mes professeurs
def Cases_Adj(event):
    global Ident, Clique
    Clique = Ident
    print(Clique)
    if Clique != Ident-1:
        showwarning("ATTENTION", "Vous ne pouvez pas cliquer ici")
    elif Clique != Ident+1:
        showwarning("ATTENTION", "Vous ne pouvez pas cliquer ici")
    elif Clique != Ident-4:
        showwarning("ATTENTION", "Vous ne pouvez pas cliquer ici")
        ...
        showwarning("ATTENTION", "Vous ne pouvez pas cliquer ici")
    elif Clique != Ident-3 or Ident+3:
        showwarning("ATTENTION", "Vous ne pouvez pas cliquer ici")
    elif Clique != Ident-5 or Ident +5:
        showwarning("ATTENTION", "Vous ne pouvez pas cliquer ici")


def Chemins(event):
    Chemin = list() # au départ puis chemin sera une liste de cases
    Ident1 = can.find_withtag(CURRENT)[0]
    Ident0 = (can.find_withtag(ListDeCan[0][0]))[0]
    print(Ident0)
    Lin_i = (Ident1-Ident0) // 4
    Col_j = (Ident1-Ident0) % 4
    Case = "(" + str(Lin_i) + "," + str(Col_j) +")"
    print(Case)
    Case_tag =  "tag1"
    Chemin.append(Case)
...
def Ajouter():
    ...
    file = open("mots_boggle.txt", "r+", encoding='utf8')
    lst=""
